  ...
  return (
    <section className="rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-1)] p-4 sm:p-5">
      <header className="mb-4">
        <h2 className="text-lg font-semibold text-[var(--text-primary)]">Участники</h2>
      </header>

      {members.length === 0 ? (
        <p className="text-sm text-[var(--text-secondary)]">Участники организации не найдены.</p>
      ) : (
          ...
          {members.map((member) => (
            ...
            >
              <div>
                ...
                <p className="text-xs text-[var(--text-secondary)]">{roleLabel(member.role)}</p>
              </div>
              {member.isOwner ? (
