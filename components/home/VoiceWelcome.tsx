...
export function VoiceWelcome({ className }: VoiceWelcomeProps) {
  ...
  function handleEnable() {
    ...
    utteranceRef.current = speakWelcomeText(VOICE_WELCOME_TEXT, {
  ...
  return (
    <div className={className}>
      <div className="flex flex-wrap items-center gap-3">
        {isPlaying ? (
          <button
            type="button"
            onClick={handleStop}
            aria-label={VOICE_WELCOME_STOP_LABEL}
          ...
          <button
            ...
            aria-label={VOICE_WELCOME_BUTTON_LABEL}
      ...
      {showTextFallback ? (
        <p
          aria-live="polite"
          className="mt-3 max-w-2xl border-l border-[var(--accent)]/30 pl-4 text-sm leading-relaxed text-[var(--text-secondary)]"
        >
          {VOICE_WELCOME_TEXT}
        </p>
      ) : null}
