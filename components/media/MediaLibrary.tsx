      ...
      <div className="relative z-[1] mx-auto w-full max-w-[1280px] px-4 pb-20 pt-6 sm:px-6 lg:px-8">
        <header className="flex flex-wrap items-end justify-between gap-5">
          <div>
            ...
            <p className="mt-4 max-w-2xl text-sm leading-6 text-white/42">
              Видео, изображения и озвучка, которые были сохранены в приватном хранилище Бизнес-Завода.
            </p>
          </div>
        ...
        {items.length === 0 ? (
          <section className="mt-6 rounded-[30px] border border-dashed border-white/[0.09] bg-white/[0.02] px-6 py-16 text-center">
            ...
            <h2 className="mt-5 text-xl font-semibold text-[#fff8e7]">Медиатека пока пустая</h2>
              ...
              >
                ...
                <div className="p-4">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <p className="text-sm font-semibold text-[#fff8e7]">
                        {item.provider === 'runway'
                          ? item.kind === 'image'
                            ? 'Сгенерированное изображение'
                            : 'Сгенерированное видео'
                          : 'Сгенерированная озвучка'}
                      </p>
                      <p className="mt-1 text-[10px] text-white/28">
                        {formatDate(item.createdAt)} · {formatBytes(item.byteSize)}
                      </p>
                    </div>
                    <span className="rounded-full border border-emerald-300/10 bg-emerald-300/[0.04] px-2 py-1 text-[8px] font-bold uppercase tracking-[0.12em] text-emerald-200/70">
                      saved
                    </span>
                  </div>
